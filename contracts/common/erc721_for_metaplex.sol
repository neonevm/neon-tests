...
contract ERC721ForMetaplex is IERC165, IERC721, IERC721Metadata {
    ...
    function claim(bytes32 from, uint64 amount) external returns (bool) {
        ...
        ISPLTokenProgram.Account memory account = _splToken.getAccount(from);
        require(account.state == ISPLTokenProgram.AccountState.Initialized, "ERC721: invalid spl token account state");
