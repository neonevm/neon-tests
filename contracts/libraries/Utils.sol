...
import "../pyth/solidity-bytes-utils/contracts/BytesLib.sol";
...
library Utils {
    ...
    */
    ...
    function getHeader(uint256 feedAddress)
        internal
        view
        returns (Header memory)
    {
        require(
            QueryAccount.cache(feedAddress, DISCRIMINATOR_SIZE, HEADER_SIZE),
            "failed to update cache"
        );

        (bool success, bytes memory rawTransmissions) = QueryAccount.data(
            feedAddress,
            DISCRIMINATOR_SIZE,
            HEADER_SIZE
        );
    ...
    function getLatestRound(uint256 feedAddress)
        internal
        view
        returns (Round memory)
    {
        ...
        uint32 latestRoundCursor = leftShiftRingbufferCursor(
            header.liveCursor,
            1,
            header.liveLength
        );
        uint32 latestRoundOffset = DISCRIMINATOR_SIZE +
            HEADER_SIZE +
            TRANSMISSION_SIZE *
            latestRoundCursor;
    ...
    function getRoundbyId(
    ...
    ) internal view returns (Round memory) {
        Header memory header = getHeader(feedAddress);
        ...
        uint32 roundOffset = DISCRIMINATOR_SIZE +
            HEADER_SIZE +
            TRANSMISSION_SIZE *
            roundPosition;
    ...
    function locateRound(
        ...
        uint32 historicalLength,
        uint8 granularity
    ) public pure returns (uint32 position, uint32 correctedRoundId) {
        uint32 liveStartRoundId = saturatingSub(latestRoundId, liveLength - 1);
        uint32 historicalEndRoundId = latestRoundId -
            (latestRoundId % granularity);
        uint32 historicalStartRoundId = saturatingSub(
            historicalEndRoundId,
            granularity * (historicalLength - 1)
        );
        ...
        if (roundId >= liveStartRoundId && roundId <= latestRoundId) {
            ...
            position = leftShiftRingbufferCursor(
                liveCursor,
                offset,
                liveLength
            );
        } else if (
            roundId >= historicalStartRoundId && roundId <= historicalEndRoundId
        ) {
            // Find the closest previous round
            correctedRoundId = roundId - (roundId % granularity);
            // + 1 because we're looking for the element before the cursor
            uint32 offset = (historicalEndRoundId - correctedRoundId) /
                granularity +
                1;

            position =
                liveLength +
                leftShiftRingbufferCursor(
                    historicalCursor,
                    offset,
                    historicalLength
                );
    ...
    function getHistoricalLength(uint256 feedAddress, uint32 liveLength)
        internal
        view
        returns (uint32)
    {
        ...
        require(
            QueryAccount.cache(feedAddress, 0, 1),
            "failed to update cache"
        );

        (bool success, uint256 feedDataLength) = QueryAccount.length(
            feedAddress
        );
        require(success, "failed to query account length");

        return
            uint32(feedDataLength - DISCRIMINATOR_SIZE - HEADER_SIZE) /
            TRANSMISSION_SIZE -
            liveLength;
    }
    ...
    function leftShiftRingbufferCursor(
        uint32 currentCursor,
        uint32 leftShiftItems,
        uint32 length
    ...
    function getRound(
        ...
        uint32 offset,
        uint32 roundId
    ) private view returns (Round memory) {
        require(
            QueryAccount.cache(feedAddress, offset, TRANSMISSION_SIZE),
            "failed to update cache"
        );

        (bool success, bytes memory rawTransmission) = QueryAccount.data(
            feedAddress,
            offset,
            TRANSMISSION_SIZE
        );
    ...
    function extractRound(uint32 roundId, bytes memory rawTransmission)
        public
        pure
        returns (Round memory)
    {
        uint32 timestamp = readLittleEndianUnsigned32(
            rawTransmission.toUint32(TRANSMISSION_TIMESTAMP_OFFSET)
        );
        int128 answer = readLittleEndianSigned128(
            rawTransmission.toUint128(TRANSMISSION_ANSWER_OFFSET)
        );
        return Round(roundId, answer, timestamp);
    }

    function extractHeader(bytes memory rawTransmissions)
        public
        pure
        returns (Header memory)
    {
        return
            Header(
                ...
                bytesToString(
                    rawTransmissions.slice(
                        HEADER_DESCRIPTION_OFFSET,
                        HEADER_DESCRIPTION_LENGTH
                    )
                ),
                rawTransmissions.toUint8(HEADER_VERSION_OFFSET), // uint8 is identical in little and big endians
                readLittleEndianUnsigned32(
                    rawTransmissions.toUint32(HEADER_LATEST_ROUND_ID_OFFSET)
                ),
                readLittleEndianUnsigned32(
                    rawTransmissions.toUint32(HEADER_LIVE_LENGTH_OFFSET)
                ),
                readLittleEndianUnsigned32(
                    rawTransmissions.toUint32(HEADER_LIVE_CURSOR_OFFSET)
                ),
                readLittleEndianUnsigned32(
                    rawTransmissions.toUint32(HEADER_HISTORICAL_CURSOR_OFFSET)
                ),
    ...
    function readLittleEndianUnsigned32(uint32 input)
        private
        pure
        returns (uint32)
    {
    ...
    function readLittleEndianSigned128(uint128 input)
        private
        pure
        returns (int128)
    {
        input =
            ((input << 8) & 0xFF00FF00FF00FF00FF00FF00FF00FF00) |
            ((input >> 8) & 0x00FF00FF00FF00FF00FF00FF00FF00FF);
        input =
            ((input << 16) & 0xFFFF0000FFFF0000FFFF0000FFFF0000) |
            ((input >> 16) & 0x0000FFFF0000FFFF0000FFFF0000FFFF);
        input =
            ((input << 32) & 0xFFFFFFFF00000000FFFFFFFF00000000) |
