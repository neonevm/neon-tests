...
contract PythOracle {
    ...
    function getCurrentPrice(bytes32 id) public view returns (PythStructs.Price memory price) {
    ...
    function queryPriceFeed(bytes32 id) public view returns (PythStructs.PriceFeed memory) {
    ...
    function parseSolanaPriceAccountData(bytes32 id, bytes memory data) public pure returns (PythStructs.PriceFeed memory priceFeed) {
        ...
        data.slice(offset, 32);
        priceFeed.productId = bytes32(id);
        offset += 32;
