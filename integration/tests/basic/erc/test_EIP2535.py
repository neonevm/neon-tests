...
class TestDiamond(BaseMixin):
    ...
    @pytest.fixture(scope="class")
    def facet_cuts(
        self, web3_client, diamond_cut_facet, diamond_loupe_facet, ownership_facet
    ):
        facet_cuts = []
        for facet in [diamond_cut_facet, diamond_loupe_facet, ownership_facet]:
            facet_cuts.append(
                (facet.address, facet_cut_action["Add"], get_selectors(facet.abi))
            )
        return facet_cuts
    ...
    def test_facet_addresses(self, diamond, facet_cuts):
        addresses = self.web3_client.call_function_at_address(
            diamond.address, "facetAddresses()", None, ["address[]"]
        )
        assert len(addresses) == 3
        for i in range(3):
            assert addresses[i] == facet_cuts[i][0].lower()

    def test_facet_function_selectors(
        self, diamond, diamond_loupe_facet, diamond_cut_facet, ownership_facet
    ):
    ...
    def test_add_and_remove_function(self, diamond, diamond_cut_facet, class_account):
        ...
        facet_cuts = [
            (new_facet.address, facet_cut_action["Add"], get_selectors(new_facet.abi))
        ]
        calldata = keccak(text="diamondCut((address,uint8,bytes4[])[],address,bytes)")[
            :4
        ] + abi.encode(
        ...
        tx = self.create_tx_object(
            class_account.address, diamond.address, 0, data=calldata
        )
        ...
        assert (
            set(get_selectors(new_facet.abi)) - set(result) == set()
        ), "selectors not added"
        result = self.web3_client.call_function_at_address(
            diamond.address, "test1Func10()", None, ["string"]
        )
        assert result == "hello"
        ...
        assert set(get_selectors(new_facet.abi)) - set(remove_selectors) == set(
            result
        ), "selectors not removed"
        with pytest.raises(ContractLogicError):
            self.web3_client.call_function_at_address(
                diamond.address, "test1Func10()", None, ["string"]
            )
