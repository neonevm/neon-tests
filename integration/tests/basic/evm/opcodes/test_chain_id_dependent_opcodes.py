...
@allure.story("ChainID dependent opcodes")
@pytest.mark.usefixtures("accounts", "web3_client")
class TestChainIdDependentOpcodes:
    ...
    def test_balance_by_sol_contract(
        self, contract_sol, class_account_sol_chain, web3_client, web3_client_sol, contract_caller_sol
    ):
        ...
        contract_sol_in_neon_network = web3_client.get_deployed_contract(
            contract_sol.address, "opcodes/ChainIdDependentOpCodes"
        )
        ...
        caller_in_neon_network = web3_client.get_deployed_contract(
            contract_caller_sol.address, "opcodes/ChainIdDependentOpCodes", "ChainIdDependentOpCodesCaller"
        )

        balance = caller_in_neon_network.functions.getBalance(
            contract_sol.address, class_account_sol_chain.address
        ).call()
        assert balance == expected_balance_sol
    ...
    def test_balance_by_neon_contract(
        ...
        contract_neon_in_sol_network = web3_client_sol.get_deployed_contract(
            contract_neon.address, "opcodes/ChainIdDependentOpCodes"
        )
        balance = contract_neon_in_sol_network.functions.getBalance(class_account.address).call()
        assert balance == expected_balance_sol

        caller_in_sol_network = web3_client_sol.get_deployed_contract(
            contract_caller_neon.address, "opcodes/ChainIdDependentOpCodes", "ChainIdDependentOpCodesCaller"
        )
