...
class TestScheduledTrx:
    ...
    def test_scheduled_trx_withdraw_neon_to_solana(
        ...
        balance_before = web3_client_sol.get_balance(neon_user_for_session.checksum_address)
        ...
        balance_after = web3_client_sol.get_balance(neon_user_for_session.checksum_address)
    ...
    def test_send_simple_single_trx(
        ...
        wait_condition(
            lambda: hex(tx.nonce) in web3_client_sol.get_pending_transactions(neon_user_for_session.checksum_address),
            timeout_sec=60,
        )
    ...
    def test_create_2_tree_accounts_with_the_same_nonce(
        self, web3_client_sol, evm_loader, neon_user_for_session, treasury_pool, common_contract
    ):
        nonce = web3_client_sol.get_nonce(neon_user_for_session.checksum_address)
        call_data = decode_function_signature("setNumber(uint256)", [1])

        trx_estimate_obj = ScheduledTrxEstimateRequest(
            neon_user_for_session.checksum_address, common_contract.address, call_data
        )
        estimate_result = web3_client_sol.estimate_scheduled(
            neon_user_for_session.solana_account.pubkey(), [trx_estimate_obj]
        )
        ...
        evm_loader.create_tree_account_multiple(neon_user_for_session, treasury_pool, tree_acc.data, payer_nonce=nonce)
        with pytest.raises(AssertionError, match="transaction with the same nonce already exists"):
            evm_loader.create_tree_account_multiple(
                neon_user_for_session, treasury_pool, tree_acc.data, payer_nonce=nonce
            )
    ...
    def test_scheduled_trx_send_tokens_to_neon_chain_contract(
        self, neon_user_for_session, evm_loader, event_caller_contract, web3_client_sol, treasury_pool
    ):
        nonce = web3_client_sol.get_nonce(neon_user_for_session.checksum_address)
        ...
        trx_estimate_obj = ScheduledTrxEstimateRequest(
            neon_user_for_session.checksum_address, event_caller_contract.address, call_data, value=value
        )
        estimate_result = web3_client_sol.estimate_scheduled(
            neon_user_for_session.solana_account.pubkey(), [trx_estimate_obj]
        )
        ...
        assert event_logs[0].args.who == neon_user_for_session.checksum_address
    ...
    def test_scheduled_trx_send_tokens_to_sol_chain_contract(
        self, neon_user_for_session, evm_loader, event_caller_sol_chain, web3_client_sol, treasury_pool
    ):
        evm_loader.deposit_wrapped_sol_from_solana_to_neon(
            neon_user_for_session.solana_account,
            "0x" + neon_user_for_session.neon_address.hex(),
    ...
    def test_scheduled_trx_with_timestamp(
        ...
        for i in range(trx_count):
            trx_estimate_obj_list.append(
                ScheduledTrxEstimateRequest(neon_user_for_session.checksum_address, contract.address, call_data)
            )
        estimate_result = web3_client_sol.estimate_scheduled(
            neon_user_for_session.solana_account.pubkey(), trx_estimate_obj_list
        )
    ...
    def test_long_chain_iterative_scheduled_trx(
        self, web3_client_sol, neon_user_for_session, treasury_pool, evm_loader, json_rpc_client, counter_contract
    ):
        total_trx_count = 8
        ...
        for _ in range(total_trx_count):
            trx_estimate_obj_list.append(
                ScheduledTrxEstimateRequest(
                    neon_user_for_session.checksum_address, counter_contract.address, call_data_counter
                )
            )
        estimate_result = web3_client_sol.estimate_scheduled(
            neon_user_for_session.solana_account.pubkey(), trx_estimate_obj_list
