...
class TestNonce(BaseMixin):
    ...
    def test_send_transaction_with_low_nonce_after_several_high(self):
        ...
        for n in [nonce + 3, nonce + 1, nonce]:
            ...
            signed_tx = self.web3_client.eth.account.sign_transaction(transaction, self.sender_account.key)
            response_trx = self.proxy_api.send_rpc("eth_sendRawTransaction", [signed_tx.rawTransaction.hex()])
            trx[n] = response_trx

        time.sleep(15)  # transaction with n+3 nonce should wait when transaction with nonce = n+2 will be accepted
        receipt_trx1 = self.proxy_api.send_rpc(method="eth_getTransactionReceipt", params=[trx[n + 3]["result"]])
