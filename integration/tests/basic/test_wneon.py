...
class TestWNeon(BaseMixin):
    ...
    def get_balances(self, wneon, address):
        ...
        wneon_balance = self.web3_client._web3.fromWei(wneon.functions.balanceOf(address).call(), "ether")
    ...
    def test_deposit_and_total_supply(self, wneon):
        ...
        neon_balance_after, wneon_balance_after = self.get_balances(wneon, self.recipient_account.address)
        ...
        assert self.web3_client._web3.fromWei(wneon.functions.totalSupply().call(),
                                              "ether") == deposit_amount + deposit_amount2
    ...
    def test_withdraw(self, wneon):
        ...
        receipt = self.web3_client.send_transaction(self.recipient_account, instruction_tx)
        assert receipt["status"] == 1
    ...
    def test_transfer_and_check_token_does_not_use_spl(self, wneon, new_account, pytestconfig):
        ...
        wait_condition(lambda: self.sol_client.get_transaction(Signature.from_string(solana_trx["result"][0]),
    ...
    def test_transfer_from(self, wneon, new_account):
        ...
        self.deposit(wneon, deposit_amount, self.sender_account)
        neon_balance_sender_before, wneon_balance_sender_before = self.get_balances(wneon, self.sender_account.address)
        neon_balance_recipient_before, wneon_balance_recipient_before = self.get_balances(wneon, new_account.address)
        transfer_amount = random.randint(1, 100)
        transfer_amount_wei = self.web3_client._web3.toWei(transfer_amount, "ether")

        with pytest.raises(web3.exceptions.ContractLogicError):
            wneon.functions.transferFrom(self.sender_account.address, new_account.address,
                                         transfer_amount_wei).buildTransaction(self.make_tx_object(new_account))

        instruction_tx = wneon.functions.approve(new_account.address, transfer_amount_wei).buildTransaction(
            self.make_tx_object(self.sender_account)

        )
        receipt = self.web3_client.send_transaction(self.sender_account, instruction_tx)
        assert receipt["status"] == 1
        instruction_tx = wneon.functions.transferFrom(self.sender_account.address, new_account.address,
                                                      transfer_amount_wei).buildTransaction(
            self.make_tx_object(new_account))
