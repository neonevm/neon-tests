...
class TestScheduledTrx:
    ...
    def test_execute_scheduled_trx_from_account(
        self,
        evm_loader,
        neon_user_for_evm_test: NeonUser,
        treasury_pool,
        basic_contract,
        neon_api_client,
        operator_keypair,
        ...
        nonce = evm_loader.get_neon_nonce(neon_user_for_evm_test.neon_address, evm_loader.sol_chain_id)
        contract_data = 18
        ...
        tx = ScheduledTransaction(
            neon_user_for_evm_test.neon_address,
            None,
            nonce,
            0,
        ...
        tree_account = evm_loader.create_tree_account(neon_user_for_evm_test, treasury_pool, tx.encode())
        transaction_tree_data = neon_api_client.get_transaction_tree(neon_user_for_evm_test.neon_address.hex(), nonce)
        ...
        additional_accounts = [
            ...
            neon_user_for_evm_test.get_balance_account(evm_loader.sol_chain_id),
        ]
        ...
        evm_loader.destroy_tree_account(neon_user_for_evm_test, treasury_pool, tree_account)
        assert (
            neon_api_client.get_transaction_tree(
                neon_user_for_evm_test.neon_address.hex(), nonce
    ...
    def test_execute_scheduled_trx_from_instruction(
        ...
        result = neon_api_client.emulate(
            neon_user_for_evm_test.neon_address.hex(), contract=basic_contract.eth_address.hex(), data=data
        )
    ...
    def test_scheduled_trx_wrong_index(
        self, evm_loader, neon_user_for_evm_test: NeonUser, treasury_pool, basic_contract, operator_keypair, holder_acc
    ):
        nonce = evm_loader.get_neon_nonce(neon_user_for_evm_test.neon_address, evm_loader.sol_chain_id)
        data = abi.function_signature_to_4byte_selector("getNumber()")
        index = 1
        ...
        with pytest.raises(solana.rpc.core.RPCException, match=InstructionAsserts.TRANSACTION_TREE_INVALID_DATA):
            evm_loader.create_tree_account(neon_user_for_evm_test, treasury_pool, tx.encode())
    ...
    def test_send_sol_with_zero_fee(
        ...
        nonce = evm_loader.get_neon_nonce(account=neon_user_for_evm_test.neon_address, chain_id=evm_loader.sol_chain_id)
    ...
    def test_send_sol_with_priority_fee(
        ...
        evm_loader.deposit_wrapped_sol_from_solana_to_neon(
            neon_user_for_evm_test.solana_account,
            "0x" + neon_user_for_evm_test.neon_address.hex(),
        ...
        user_balance_before = evm_loader.get_neon_balance(neon_user_for_evm_test.neon_address, evm_loader.sol_chain_id)
        treasury_balance_before = evm_loader.get_solana_balance(treasury_pool_new.account)

        tree_account = evm_loader.create_tree_account(neon_user_for_evm_test, treasury_pool_new, tx.encode())
        ...
        wait_condition(
            lambda: evm_loader.get_neon_balance(neon_user_for_evm_test.neon_address, evm_loader.sol_chain_id)
            < user_balance_before
        )

        user_balance_after = evm_loader.get_neon_balance(neon_user_for_evm_test.neon_address, evm_loader.sol_chain_id)
        ...
        emulate_result = neon_api_client.emulate(
            neon_user_for_evm_test.neon_address.hex(),
        ...
        user_balance_before_destroy = evm_loader.get_neon_balance(
            neon_user_for_evm_test.neon_address, evm_loader.sol_chain_id
        )

        evm_loader.destroy_tree_account(neon_user_for_evm_test, treasury_pool_new, tree_account)
