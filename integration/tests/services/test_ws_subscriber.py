...
class TestSubscriber:
    ...
    def call_contract_events(self, sender_account, event_caller_contract):
    ...
    async def test_subscribe_to_newheads(self, ws_subscriber_url):
        ...
        async with websockets.connect(ws_subscriber_url) as ws:
            ...
            self.web3_client.send_neon(sender_account, recipient_account, 0.1)
            messages = await ws_receive_all_messages(ws)
            assert (
                len(messages) == 0
    ...
    async def test_logs(self, param_fields, event_caller_contract, ws_subscriber_url):
        ...
        async with websockets.connect(ws_subscriber_url) as ws:
            ...
            if len(param_fields) == 0:
                ...
                assert (
                    messages[0].params.result.address == event_caller_contract.address.lower()
            ...
            assert_fields_are_hex(
                response.params.result,
                ["transactionHash", "blockHash", "blockNumber", "transactionIndex", "address", "logIndex", "data"],
            )
    ...
    async def test_filter_log_by_topics(
        ...
        async with websockets.connect(ws_subscriber_url) as ws:
            ...
            self.call_contract_events(sender_account, event_caller_contract)

            # validate result — all the received messages
            messages = await ws_receive_all_messages(ws)
            assert len(messages) == log_count, f"Expected {log_count} event logs, but found {len(messages)}: {messages}"
    ...
    async def test_multiples_users_different_logs(self, event_caller_contract, ws_subscriber_url):
        ...
        async with (websockets.connect(ws_subscriber_url) as ws1, websockets.connect(ws_subscriber_url) as ws2):
            ...
            assert (
                r_subscription1 == subscription1
            ), f"expected received subscription {r_subscription1} to be equal {subscription1}"
            assert (
                messages2[0].params.subscription == subscription2
