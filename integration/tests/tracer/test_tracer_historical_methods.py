...
class TestTracerHistoricalMethods:
    ...
    def test_eth_call(self, storage_contract, request_type):
        ...
        tx_obj_3, request_value_3, _ = call_storage(
            sender_account, storage_contract, store_value_3, request_type, self.web3_client
        )
        wait_condition(
            lambda: int(
                self.tracer_api.send_rpc(
    ...
    def test_eth_get_balance(self, request_type):
        ...
        sender_balance_after = self.web3_client.get_balance(self.sender_account.address)
    ...
    def test_check_neon_revision(self):
        revision = self.tracer_api.send_rpc(method="get_neon_revision", params={"slot": 1})
