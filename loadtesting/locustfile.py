...
def statistics_collector(func: tp.Callable) -> tp.Callable:
    ...
    def wrap(*args, **kwargs) -> tp.Any:
        ...
        requset_type = f"`{func.__name__.replace('_', ' ')}`"
        event: tp.Dict[str, tp.Any] = dict(task_id=task_id, request_type=requset_type)
        ...
        try:
            ...
            LOG.error(f"web3 RPC call {requset_type} is failed: {err}")
