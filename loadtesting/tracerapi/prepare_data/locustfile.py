...
class EthGetStorageAtPreparationStage(head.NeonTasksSet):
    ...
    def deploy_storage_contract(self):
        ...
        contract, _ = self.deploy_contract(contract_name, RETRIEVE_STORE_VERSION, account, contract_name="Storage")
        if not contract:
...
class NeonTransferPreparationStage(head.NeonTasksSet):
    ...
    def send_neon(self):
        recipient_balance_before, sender_balance_before = super(NeonTransferPreparationStage, self).get_balances()
        tx, nonce = super(NeonTransferPreparationStage, self).task_send_neon()
        recipient_balance_after, sender_balance_after = super(NeonTransferPreparationStage, self).get_balances()
        info = {
...
class ERC20TransferPreparationStage(head.ERC20TasksSet):
    ...
    def send_erc20(self):
        recipient_balance_before, sender_balance_before = super(ERC20TransferPreparationStage, self).get_balances()
        tx, nonce = super(ERC20TransferPreparationStage, self).task_send_erc20()
        recipient_balance_after, sender_balance_after = super(ERC20TransferPreparationStage, self).get_balances()
        info = {
            "sender_balance_before": f"{sender_balance_before}",
            "sender_balance_after": f"{sender_balance_after}",
            "sender_nonce": f"{nonce}",
...
class ERC20WrappedPreparationStage(head.ERC20SPLTasksSet):
    ...
    def send_erc20spl(self):
        recipient_balance_before, sender_balance_before = super(ERC20WrappedPreparationStage, self).get_balances()
        tx, nonce = super(ERC20WrappedPreparationStage, self).task_send_erc20_spl()
        recipient_balance_after, sender_balance_after = super(ERC20WrappedPreparationStage, self).get_balances()
        info = {
            "sender_balance_before": f"{sender_balance_before}",
            "sender_balance_after": f"{sender_balance_after}",
            "sender_nonce": f"{nonce}",
