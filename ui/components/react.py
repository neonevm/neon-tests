...
class Button:
    def __init__(self, page: Page, text: tp.Optional[str] = None, selector: tp.Optional[str] = None) -> None:
        self.page = page
...
class Combobox:
    ...
    def __init__(
        ...
        self.page = page
        if placeholder:
            selector = f'//div[contains(@class, "style__placeholder") and text() = "{placeholder}"]/../..'
        elif label:
...
class RefreshModal:
    ...
    def refresh(self) -> None:
        ...
        for attempt in range(5):
            button.click()
            try:
                loader = self.page.wait_for_selector(selector='//div[text()="LOADING"]', state="attached")
